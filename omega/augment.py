...
class AbstractAugment:
    ...
    def __call__(self, query: str) -> str:
        try:
            ...
            log.info(f"Augmented query: '{query}' -> '{new_query}'")
            return new_query
...
class NoAugment(AbstractAugment):
    def __init__(self, **kwargs):
        log.info("Running no query augmentation")
...
class LocalLLMAugment(AbstractAugment):
    def __init__(self, **kwargs):
        ...
        log.info(f"Running query augmentation with local LLM {model_name} (thanks Nous!)")
...
class OpenAIAugment(AbstractAugment):
    def __init__(self, **kwargs):
        self.client = OpenAI()
        log.info("Running query augmentation with OpenAI GPT-4")
